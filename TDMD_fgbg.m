...
% ----- (B) Mild vignetting / global gradient -----
...
mp = 2^nextpow2(2*m);                 % e.g. 128 when m=60
...
% inverse Haar along mode-3
Xbg_haar_tmp = permute(V_bg_haar_detr_pad, [1 3 2]);      % n x T x L
Xbg_haar_tmp = mode3_matmul(Xbg_haar_tmp, H);             % n x T x L
V_bg_haar_detr_pad_fullT = permute(Xbg_haar_tmp, [1 3 2]);% n x L x T
V_bg_haar_detr_fullT = V_bg_haar_detr_pad_fullT(:,1:m,:); % crop
V_bg_haar_detr = V_bg_haar_detr_fullT(:,:,1:T-1);

% ---- Add back row background and form FG ----
V_bg_fft  = real(V_bg_fft_detr(:,:,1:T-1)  + BG_row(:,:,1:T-1));
V_fg_fft  = max(real(V(:,:,1:T-1) - V_bg_fft), 0);

V_bg_cos  = real(V_bg_cos_detr(:,:,1:T-1)  + BG_row(:,:,1:T-1));
V_fg_cos  = max(real(V(:,:,1:T-1) - V_bg_cos), 0);

V_bg_haar = real(V_bg_haar_detr(:,:,1:T-1) + BG_row(:,:,1:T-1));
...
figure('Color','w','Position',[100 100 1650 520]); 
colormap(gray);

for j = 1:5
    subplot(2,5,j);
    imagesc(real(FGs{j}) + 0.25);
    axis image off; 
    set(gca,'CLim',[0 1]);
...
%% 9) Full reconstruction for 5 methods
...
Xrec_haar_tmp = permute(V_rec_haar_coeff_fullT, [1 3 2]);         % n x T x L
Xrec_haar_tmp = mode3_matmul(Xrec_haar_tmp, H);                   % inverse Haar
V_rec_haar_detr_pad_fullT = permute(Xrec_haar_tmp, [1 3 2]);      % n x L x T
...
%% 10) Per-frame relative reconstruction error
...
switch lower(which)
    ...
    case 'cos'
        Xbg = real(idct(Zbg, [], 3));              % n x T x m3
        V_bg_detr_fullT = permute(Xbg, [1 3 2]);   % n x m3 x T

    case 'haar'
        % return coeff-domain; caller must inverse using H
        Xbg = Zbg;
...
% Input fX,fY: n x r x m3
% Output V_rec_detr_fullT: n x m3 x T
...
switch lower(which)
    ...
    case 'haar'
        % return coeff-domain; caller must inverse using H
        V_rec_detr_fullT = permute(Zrec, [1 3 2]);  % n x m3 x T
