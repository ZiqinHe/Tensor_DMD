...
% Output: per-frame relative error table for all frames t=1..r
...
%% =====================================================================
% 4) Unfolding DMD via WHOLE bcirc matrix (use full (nm) x (r*m))
%     -> produce per-frame recon as nm-vector (reshape to n x m)
% =====================================================================
...
%% =====================================================================
% 5) TT-DMD (use your exact block: Xmat = Xb(:,1:r), Ymat = Yb(:,1:r))
%     -> produce per-frame recon: n x m x r
% =====================================================================
Xmat = Xb(:, 1:r);
Ymat = Yb(:, 1:r);
...
G   = core2cell(Xtt);
for k = 1:numel(G), G{k} = ensure_core3(G{k}); end
...
function Gout = tt_left_orth_cells(Gin)
    ...
    for k = 1:d-1
        ...
        Gnext_mat = reshape(Gnext, rkp1, []);     % important (MATLAB compat)
...
% --- shared color limits (optional but recommended) ---
all_rec = []; all_err = [];
for j = 1:5
...
for j = 1:5
    nexttile(j);
    imagesc(R{j}, cl_rec); axis image off;
...
for j = 1:5
    nexttile(5+j);
    imagesc(E{j}, cl_err); axis image off;
    % 如果你想第二行也有标题，取消下一行注释：
...
%% --- local clim helper (put near bottom if not already present) ---
function clim = local_clim_safe(A)
    A = double(A(:));
    A = A(isfinite(A));
    if isempty(A), clim = [0 1]; return; end
    q = quantile(A,[0.02 0.98]);
    if ~isfinite(q(1)) || ~isfinite(q(2)) || q(2) <= q(1)
        clim = [min(A), max(A)];
        if clim(2) <= clim(1), clim = [0 1]; end
    else
        clim = q;
    end
end
